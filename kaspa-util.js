...
function scriptPublicKeyToAddress(spkBuffer, prefix) {
    if (spkBuffer[0] == 0x20) {
        return publicKeyToAddress(spkBuffer.slice(1, 33), prefix, 'schnorr');
    } else if (spkBuffer[0] == 0x21) {
        return publicKeyToAddress(spkBuffer.slice(1, 34), prefix, 'ecdsa');
    } else if (spkBuffer[0] == 0xaa) {
        return publicKeyToAddress(spkBuffer.slice(2, 34), prefix, 'p2sh');
    }

    throw new Error('Invalid SPK');
}
...
function addressToScriptPublicKey(address) {
